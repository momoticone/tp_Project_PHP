...
function modifierPayloadEnObjet(array $data, array $file):Produit
{
    $fichier = enregistrerFichierEnvoye($file["product-photo-file"]);
    $produit->nom = $data['product-name'];
    $produit->prix = $data['product-price'];
...
function create()
{
 ...
 // rediriger "/produit/ajouter.html.php" ver le bon chemin acces :'/../../views/produit/ajouter.html.php'
...
function edit()
{
    ...
    if (!isset($_GET['id']))
    {
        ...
        if ($messageErreur === null)
        {
            ...
            $produit = modiferPayloadEnObjet($_POST, $_FILES);
            $produit->save();
            onVaRediriger('/catalogue');
        }
    }
